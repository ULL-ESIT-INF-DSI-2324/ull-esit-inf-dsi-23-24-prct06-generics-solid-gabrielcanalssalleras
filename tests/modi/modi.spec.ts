...
describe('Search & Collect', () => {
  ...
  it('Ambas instancias extienden de la colección de búsqueda abstracta (responden a sus métodos)', () => {
	...
	it('La colección numérica puede eliminar números', () => {
		...
		expect(numSerchableCollection.items).to.deep.equal([10, 42]);
	})
	...
	it('La colección numérica puede buscar números', () => {
		expect(numSerchableCollection.search(42)).to.eql([1]);
		expect(numSerchableCollection.search(10)).to.eql([0]);
		expect(numSerchableCollection.search(37)).to.eql([]);
	});

	it('La colección de strings puede buscar strings', () => {
		expect(strSerchableCollection.search("ojo")).deep.equal(["rojo"]);
	...
	it('Se puede obtener un elemento de la colección numérica', () => {
		expect(numSerchableCollection.getItem(0)).to.equal(10);
		expect(numSerchableCollection.getItem(1)).to.equal(42);
		expect(numSerchableCollection.getItem(2)).to.equal(undefined);
	}),

	it('Se puede obtener un elemento de la colección de strings', () => {
		expect(strSerchableCollection.getItem(0)).to.equal("palabra");
		expect(strSerchableCollection.getItem(1)).to.equal("rojo");
		expect(strSerchableCollection.getItem(2)).to.equal(undefined);
	}),
	...
	it('Se puede eliminar un elemento de la colección numérica', () => {
		...
		expect(numSerchableCollection.getNumberOfItems()).to.equal(1);
		expect(numSerchableCollection.items).to.eql([42]);
	}),
